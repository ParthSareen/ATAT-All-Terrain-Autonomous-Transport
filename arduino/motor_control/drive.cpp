...
float * Drive::encoderReading(unsigned long *lastTime, float speed = MAX_SPEED){
  ...
  sim_rpm_left = (-(0.5795)*exp(0.02228 * pulse_read_left)) + 156;
  sim_rpm_right = (-(0.5795)*exp(0.02228 * pulse_read_right)) + 156;
  measured_rpm_left = 111111 / pulse_read_left;
  measured_rpm_right = 111111 / pulse_read_right;
  Serial.println("MeasuredRPM");
  Serial.println(measured_rpm_right);

  if (abs(sim_rpm_left - measured_rpm_left) > 5){
    distance_left = totalMinutes * sim_rpm_left * (2 * PI * radius);
    
  }else{
    distance_left = totalMinutes * measured_rpm_left * (2 * PI * radius);
  }

 if (abs(sim_rpm_right - measured_rpm_right) > 5){
    distance_right = totalMinutes * sim_rpm_right * (2 * PI * radius);
  }else{
    distance_right = totalMinutes * measured_rpm_right * (2 * PI * radius);
  }
